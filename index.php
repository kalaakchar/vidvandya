<?php require('top.php'); ?>
<div class="vid-banner-area h-100 ">
    <div class="container h-100">
        <div class="row d-flex align-items-center h-100">
            <div class="col-lg-7">
                <div class="banner-text wow fadeInLeft slow">
                    <h1><span class="sp-col-1">Lorem</span><br><span class="sp-col-2">Ipsum</span> & <span class="sp-col-2">Grow your Business</span> with us.</h1>
                    <a href="#" class="btn">start a project</a>
                </div>
            </div>
            <div class="col-lg-5">
                <div class="banner-outer">
                    <div class="banner-img wow zoomIn">
                        <div class="banner-full">
                            <img src="img/slider/banner-vid.png" alt="banner-image">
                        </div>
                        <div class="banner-up-1">
                            <img src="img/slider/banner-vid-1.png" alt="banner-image">
                        </div>
                        <div class="banner-up-2">
                            <img src="img/slider/banner-vid-2.png" alt="banner-image">
                        </div>
                        <div class="banner-up-3">
                            <img src="img/slider/banner-vid-3.png" alt="banner-image">
                        </div>
                    </div>
                    <div class="banner-out-1">
                        <img src="img/slider/banner-out-1.png" alt="Image">
                    </div>
                    <div class="banner-out-2">
                        <img src="img/slider/banner-out-2.png" alt="Image">
                    </div>
                    <div class="banner-out-3">
                        <img src="img/slider/banner-out-3.png" alt="Image">
                    </div>
                    <div class="banner-out-4">
                        <img src="img/slider/banner-out-4.png" alt="Image">
                    </div>
                    <div class="banner-out-5">
                        <img src="img/slider/banner-out-5.png" alt="Image">
                    </div>
                </div>
            </div>
        </div>
        <div class="vid-anim">
            <div class="square-1 anim-1 position-absolute"></div>
            <div class="square-1 anim-3 position-absolute"></div>
            <div class="triangle anim-2 position-absolute">
                <img src="img/angle.png" alt="angle">
            </div>
            <div class="cross anim-4 position-absolute">
                <img src="img/cross.png" alt="cross">
            </div>
            <div class="cross anim-5 position-absolute">
                <img src="img/cross.png" alt="cross">
            </div>
            <div class="cross anim-6 position-absolute">
                <img src="img/half-circle.png" alt="half-circle">
            </div>

        </div>
    </div>
</div>
<div class="vid-abt mob-none">
    <div class="vid-abt-det">
        <div class="container vid-abt-img">
            <div class="row">
                <div class="col-md-5">
                    <div class="vid-abt-lft">
                        <div class="vid-abt tlt">
                            <h5>WHY CHOOSE US</h5>
                        </div>
                        <div class="vid-abt-sb">
                            <h2>Boosts Your Business</h2>
                        </div>
                        <div class="vid-abt-txt">
                            <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Nulla cum iusto labore
                                accusamus magni nam dolorum voluptatem obcaecati unde pariatur?</p>
                        </div>

                    </div>
                </div>
                <div class="col-md-7">

                    <img src="img/we.gif" alt="">
                    <!-- <div class="row">
                        <div class="col-md-6 vid-hov">
                            <div class="vid-abt-rgh">
                                <div class="vid-rgh-img">
                                    <span class="vid-icn">
                                        <i class="fa fa-solid fa-briefcase"></i>
                                    </span>
                                </div>
                                <div class="vid-rgh-txt">
                                    <h5>All Sizes Business</h5>
                                    <p>Lorem ipsum dolor sit amet.</p>
                                </div>

                            </div>

                        </div>
                        <div class="col-md-6 vid-hov">
                            <div class="vid-abt-rgh">
                                <div class="vid-rgh-img">
                                    <span class="vid-icn">
                                        <i class="fa-solid fa-briefcase"></i>
                                    </span>
                                </div>
                                <div class="vid-rgh-txt">
                                    <h5>All Sizes Business</h5>
                                    <p>Lorem ipsum dolor sit amet.</p>
                                </div>

                            </div>

                        </div>
                        <div class="col-md-6 vid-hov">
                            <div class="vid-abt-rgh">
                                <div class="vid-rgh-img">
                                    <span class="vid-icn">
                                        <i class="fa-solid fa-briefcase"></i>
                                    </span>
                                </div>
                                <div class="vid-rgh-txt">
                                    <h5>All Sizes Business</h5>
                                    <p>Lorem ipsum dolor sit amet.</p>
                                </div>

                            </div>

                        </div>
                        <div class="col-md-6 vid-hov">
                            <div class="vid-abt-rgh">
                                <div class="vid-rgh-img">
                                    <span class="vid-icn">
                                        <i class="ti-arrow-up"></i>
                                    </span>
                                </div>
                                <div class="vid-rgh-txt">
                                    <h5>All Sizes Business</h5>
                                    <p>Lorem ipsum dolor sit amet.</p>
                                </div>

                            </div>

                        </div>
                    </div> -->

                </div>
            </div>
        </div>
    </div>
</div>
<div class="vid-ser mob-none">
    <div class="container">
        <div class="serv-tp">
            <div class="tlt">
                <h5>WHY CHOOSE US</h5>
            </div>
            <div class="feed-head-tlt">
                <h2>What We Offer</h2>
            </div>

        </div>
        <div class="row">
            <div class="col-md-3 col-sm-6 hov-tp">
                <div class="vid-serv">
                    <div class="serv">
                        <div class="serv-img">
                            <img src="img/icon/web.png" alt="">
                        </div>
                        <div class="serv-tlt">
                            <h2>Web Designing</h2>
                        </div>
                    </div>
                    <div class="serv-txt">
                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Aperiam, consequuntur.</p>
                    </div>
                </div>

            </div>
            <div class="col-md-3 col-sm-6 mt-10">
                <div class="vid-serv">
                    <div class="serv">
                        <div class="serv-img">
                            <img src="img/icon/dev.png" alt="">
                        </div>
                        <div class="serv-tlt">
                            <h2>Web Development</h2>
                        </div>
                    </div>

                    <div class="serv-txt">
                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Aperiam, consequuntur.</p>
                    </div>
                </div>

            </div>
            <div class="col-md-3 col-sm-6 hov-tp">
                <div class="vid-serv">
                    <div class="serv">
                        <div class="serv-img">
                            <img src="img/icon/graphic.png" alt="">
                        </div>
                        <div class="serv-tlt">
                            <h2>Graphic Designing</h2>
                        </div>
                    </div>
                    <div class="serv-txt">
                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Aperiam, consequuntur.</p>
                    </div>
                </div>

            </div>
            <div class="col-md-3 col-sm-6 mt-10">
                <div class="vid-serv">
                    <div class="serv">
                        <div class="serv-img">
                            <img src="img/icon/seo.png" alt="">
                        </div>
                        <div class="serv-tlt">
                            <h2>SEO Optimization</h2>
                        </div>
                    </div>
                    <div class="serv-txt">
                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Aperiam, consequuntur.</p>
                    </div>
                </div>

            </div>
        </div>
    </div>
</div>
<div class="vid-wbp  wbp-wrapper-2 section-padding mmb-50 mob-none">
    <div class="container-fluid" style="width: 94%;">
        <div class="row justify-content-center">
            <div class="col-lg-6">
                <div class="text-head text-center wbp-head">
                    <h1>Our <span class="sp-col-2">Development <br></span> Process</h1>
                </div>
            </div>
        </div>
        <div class="vid-wbp-post">
            <div class="row">
                <div class="col-lg-2 col-md-3">
                    <div class="single-wbp-post wbp-position-1">
                        <div class="vid-wbp-icon sp-col-1">
                            <img src="img/gif/chat.gif" alt="">
                            <h4>Discussion</h4>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-3">
                    <div class="single-wbp-post wbp-position-2">
                        <div class="vid-wbp-icon sp-col-2">
                            <img src="img/gif/rese.gif" alt="">
                            <h4>Research</h4>
                        </div>
                    </div>
                </div>
                <div class="col-lg-2 col-md-3">
                    <div class="single-wbp-post wbp-position-3">
                        <div class="vid-wbp-icon sp-col-5">
                            <img src="img/gif/design.gif" alt="">
                            <h4>Design</h4>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 col-md-3">
                    <div class="single-wbp-post wbp-position-4">
                        <div class="vid-wbp-icon sp-col-3">
                            <img src="img/gif/present.gif" alt="">
                            <h4>Present</h4>
                        </div>
                    </div>
                </div>
                <div class="col-lg-2 col-md-3">
                    <div class="single-wbp-post wbp-position-5">
                        <div class="vid-wbp-icon sp-col-3">
                            <img src="img/gif/launch.gif" alt="">
                            <h4>Launch Project</h4>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="feed mob-none">
    <div class="feed-det">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <div class="feed-lft">
                        <div class="feed-lft-dlt">
                            <div class="feed-lft-tlt">
                                <h5>Testimonials</h5>

                            </div>
                            <div class="feed-head-tlt">
                                <h2>What Our Clients Say</h2>
                            </div>
                        </div>
                        <div class="feed-bg">
                            <div class="feed-bg-img">
                                <img src="img/quote.png" alt="">
                            </div>

                        </div>
                    </div>
                </div>

                <div class="col-md-8">
                    <div class="feed-rgh">
                        <div class="feed-rgh-st one-time">
                            <div class="feed-rgh-1">

                                <div class="feed-bg-st">
                                    <div class="feed-rgh-qut">
                                        <img src="img/quotat.png" alt="">
                                    </div>
                                </div>
                                <div class="feed-text">
                                    <p>Design is a way of life, a point of view. It involves the whole complex of visual
                                        commun ications: talen.t, creative ability manual skill.</p>
                                </div>
                                <div class="feed-auth">
                                    <div class="auth-pht">
                                        <img src="img/quotation.png" alt="">
                                    </div>
                                    <div class="auth-nme">
                                        <div class="auth-tlt">
                                            Name
                                        </div>
                                        <div class="auth-cmp">
                                            Company Name
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="feed-rgh-1">

                                <div class="feed-bg-st">
                                    <div class="feed-rgh-qut">
                                        <img src="img/quotat.png" alt="">
                                    </div>
                                </div>
                                <div class="feed-text">
                                    <p>Design is a way of life, a point of view. It involves the whole complex of visual
                                        commun ications: talen.t, creative ability manual skill.</p>
                                </div>
                                <div class="feed-auth">
                                    <div class="auth-pht">
                                        <img src="img/quotation.png" alt="">
                                    </div>
                                    <div class="auth-nme">
                                        <div class="auth-tlt">
                                            Name
                                        </div>
                                        <div class="auth-cmp">
                                            Company Name
                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>


                </div>
            </div>
        </div>
    </div>

</div>

<section class="sect-pad-tb portf mob-none">
    <div class="container-fluid">
        <h2>Recent Projects</h2>
        <div class="row center">
            <div class="col-md-8">
                <div class="ptf-dt">
                    <div class="ptf-img">
                        <img src="img/port/1.png" alt="">
                    </div>


                </div>
            </div>
            <div class="col-md-8">
                <div class="ptf-dt">
                    <div class="ptf-img">
                        <img src="img/port/1.png" alt="">
                    </div>


                </div>
            </div>


            <div class="col-md-8">
                <div class="ptf-dt">
                    <div class="ptf-img">
                        <img src="img/port/1.png" alt="">
                    </div>


                </div>
            </div>
            <div class="col-md-8">
                <div class="ptf-dt">
                    <div class="ptf-img">
                        <img src="img/port/1.png" alt="">
                    </div>


                </div>
            </div>

        </div>
    </div>
</section>

<section class="sect-pad-tb who-work-with mob-none">
    <div class="container">
        <div class="row">
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-5 align-self-center">
                <h4 class="main-title mb-0 mt-60 title-who-work-with c-white">Who We Work With</h4>
                <p class=" mb-0 mt-3 pr-0 pr-md-0 pr-lg-5 mr-0 mr-md-0 mr-lg-4 c-white">As a leading
                    development company in India, we worked with 20+ businesses either it is a start-up or
                    enterprise and delivers the best solution in the industry. At Hyperlink InfoSystem, we offer a broad
                    range of development services based on business requirements.</p>
            </div>
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-7 mt-3">
                <div class="row">
                    <div class="col-6 col-sm-6 col-md-6 col-xl-6 my-3">
                        <div class="card-box p-3 p-md-4 text-center">
                            <svg class="mb-3" version="1.1" id="Capa_81" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512" xml:space="preserve" height="60px" width="60px" fill="#1d13ed">
                                <g>
                                    <g>
                                        <path d="M351.117,288.678l-69.513-42.712c-0.847-0.521-1.821-0.874-2.804-1.028l-21.763-3.84c-4.026-0.712-7.98,2.058-8.69,6.084 s2.057,7.98,6.083,8.69l20.537,3.624l68.297,41.965c3.334,2.049,7.907,1.054,10.12-2.166 C355.758,295.839,354.686,290.873,351.117,288.678z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M509.513,291.083c-2.83-3.027-7.576-3.187-10.603-0.359l-30.409,28.42L329.163,181.662L509.161,12.974 c3.022-2.833,3.176-7.58,0.344-10.602c-2.832-3.022-7.58-3.176-10.602-0.344l-185.69,174.022 c-3.062,2.869-3.126,7.865-0.139,10.813l12.325,12.159l-3.934,3.474l-82.381-7.094c-1.271-0.108-2.592,0.111-3.752,0.647 l-65.273,29.728c-0.19-0.049-0.38-0.097-0.577-0.132l-37.546-6.626l3.036-7.761c1.501-3.838-0.374-8.168-4.2-9.697L10.756,153.555 c-3.848-1.541-8.212,0.332-9.75,4.179c-1.539,3.846,0.332,8.212,4.179,9.75l113.103,45.242L51.636,383.055l-41.162-14.579 c-3.906-1.382-8.192,0.661-9.575,4.567c-1.383,3.905,0.662,8.192,4.567,9.575l48.006,17.002c3.768,1.335,8.034-0.618,9.489-4.338 L74.1,366.816l29.187,16.356l-3.219,5.218c-9.121,14.783-4.516,34.231,10.268,43.353c7.818,4.824,17.523,5.968,26.259,3.14 c1.401,9.104,6.751,17.228,14.596,22.068c7.818,4.824,17.522,5.968,26.259,3.14c1.402,9.104,6.75,17.227,14.595,22.068 c7.842,4.839,17.496,5.978,26.261,3.149c1.404,9.1,6.751,17.22,14.594,22.059c11.399,7.033,26.487,5.982,36.762-2.63 c2.59-2.171,4.819-4.762,6.593-7.638l5.117-8.292l4.213,2.588c11.414,7.013,26.498,5.933,36.757-2.698 c2.586-2.176,4.81-4.771,6.579-7.65c3.516-5.723,4.971-12.137,4.587-18.381c2.382,0.573,4.82,0.881,7.28,0.881 c10.894,0,21.199-5.745,26.9-15.023c3.434-5.59,5.003-11.963,4.605-18.378c2.376,0.57,4.809,0.876,7.261,0.876 c10.894,0,21.2-5.745,26.9-15.023c3.437-5.594,5.005-11.971,4.604-18.391c2.402,0.572,4.841,0.871,7.269,0.871 c10.594,0,20.953-5.334,26.894-15.003c9.094-14.801,4.452-34.24-10.349-43.334l-5.14-3.158l17.239-18.012l16.125,15.91 c2.828,2.789,7.489,2.851,10.39,0.141l35.671-33.337C512.182,298.857,512.342,294.11,509.513,291.083z M140.923,413.597 c-4.779,7.743-14.966,10.155-22.71,5.379c-7.744-4.778-10.156-14.965-5.379-22.709l42.014-68.091 c4.689-7.6,14.751-10.085,22.434-5.546c7.95,4.7,10.144,15.599,5.371,23.336L140.923,413.597z M181.776,438.805 c-4.777,7.743-14.965,10.156-22.708,5.379c-7.65-4.721-10.107-15.047-5.379-22.709l33.61-54.473 c4.689-7.599,14.751-10.085,22.434-5.546c7.95,4.7,10.144,15.599,5.37,23.336L181.776,438.805z M222.634,464.013 c-2.316,3.751-5.954,6.377-10.242,7.393c-4.291,1.017-8.718,0.3-12.469-2.014c-7.649-4.721-10.106-15.048-5.378-22.709 l25.208-40.855c4.689-7.6,14.752-10.085,22.434-5.546c7.95,4.7,10.144,15.599,5.371,23.336L222.634,464.013z M280.01,462.446 l-16.522,26.776c-4.721,7.65-15.047,10.105-22.71,5.378c-7.649-4.72-10.105-15.048-5.378-22.709l16.805-27.236 c4.689-7.598,14.752-10.087,22.434-5.545C282.589,443.809,284.783,454.709,280.01,462.446z M416.654,328.948l-37.182-22.847 c-3.448-2.119-8.199-0.984-10.317,2.464c-2.119,3.448-0.984,8.199,2.464,10.317l55.397,34.039 c7.752,4.764,10.184,14.946,5.421,22.699c-4.764,7.752-14.947,10.184-22.7,5.421l-59.413-36.508 c-3.529-2.17-8.149-1.065-10.317,2.464c-2.168,3.529-1.065,8.149,2.464,10.317l45.78,28.13 c7.659,4.706,10.134,15.029,5.421,22.699c-4.706,7.659-15.029,10.133-22.699,5.421l-41.488-25.493 c-3.53-2.169-8.149-1.065-10.317,2.464c-2.168,3.529-1.065,8.149,2.464,10.317l27.854,17.115 c7.658,4.707,10.134,15.028,5.421,22.699c-4.706,7.658-15.029,10.134-22.699,5.421c0,0-24.496-15.037-24.725-15.147 c-3.425-1.654-7.613-0.477-9.645,2.831c-2.168,3.529-1.065,8.149,2.464,10.317l10.419,6.402 c7.752,4.764,10.183,14.946,5.42,22.699c-4.706,7.659-15.029,10.133-22.699,5.421l-4.188-2.574l3.527-5.717 c4.509-7.308,5.908-16.308,3.838-24.692c-2.042-8.274-7.136-15.176-14.341-19.435c-5.622-3.322-11.911-4.668-18.035-4.255 c1.15-4.94,1.156-10.146-0.076-15.137c-2.042-8.274-7.136-15.176-14.341-19.435c-5.622-3.323-11.911-4.669-18.035-4.256 c3.093-13.288-2.601-27.585-14.417-34.571c-5.622-3.322-11.911-4.668-18.035-4.256c3.093-13.288-2.601-27.585-14.417-34.571 c-14.669-8.671-33.884-3.922-42.834,10.582l-30.91,50.096L79.62,352.711l46.738-119.442l25.448,4.491 c-7.238,8.99-9.273,21.647-4.173,32.848c5.278,11.588,16.744,18.445,28.718,18.444c4.361,0,8.79-0.91,13.011-2.832l43.928-20.008 c3.72-1.694,5.412-6.215,3.717-9.935c-1.694-3.719-6.215-5.413-9.935-3.717l-43.929,20.007 c-8.279,3.772-18.086,0.103-21.858-8.178c-3.771-8.281-0.103-18.086,8.178-21.858l70.294-32.015l83.618,7.201 c0.215,0.019,0.43,0.028,0.644,0.028c1.82,0,3.587-0.663,4.964-1.877l7.118-6.285l100.186,98.849L416.654,328.948z">
                                        </path>
                                    </g>
                                </g>
                            </svg>
                            <h3 class="work-title h5 mb-0"><b>Start Up Business</b></h3>
                        </div>
                    </div>
                    <div class="col-6 col-sm-6 col-md-6 col-xl-6 my-3">
                        <div class="card-box p-3 p-md-4 text-center">
                            <svg class="mb-3" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512" xml:space="preserve" height="60px" width="60px" fill="#4caf50">
                                <g>
                                    <g>
                                        <path d="M488.399,492h-21.933V173.536c0-14.823-12.06-26.882-26.882-26.882H390.56c-14.823,0-26.882,12.06-26.882,26.882V492 h-55.692V317.825c0-14.823-12.059-26.882-26.882-26.882H232.08c-14.823,0-26.882,12.06-26.882,26.882V492h-55.692v-90.204 c0-14.823-12.06-26.882-26.882-26.882H73.599c-14.823,0-26.882,12.06-26.882,26.882V492H23.601c-5.523,0-10,4.477-10,10 s4.477,10,10,10h464.798c5.523,0,10-4.477,10-10S493.922,492,488.399,492z M129.504,492H66.716v-90.204 c0-3.795,3.087-6.882,6.882-6.882h49.024c3.795,0,6.882,3.087,6.882,6.882V492z M287.985,492h-62.788V317.825 c0-3.795,3.087-6.882,6.882-6.882h49.024c3.794,0,6.882,3.087,6.882,6.882V492z M446.466,492h-62.788V173.536 c0-3.795,3.087-6.882,6.882-6.882h49.024c3.795,0,6.882,3.087,6.882,6.882V492z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M466.442,10.516c0.14-2.729-0.82-5.504-2.904-7.588c-2.084-2.084-4.859-3.045-7.588-2.904 C455.789,0.017,455.63,0,455.466,0h-60.5c-5.523,0-10,4.477-10,10s4.477,10,10,10h37.357l-98.857,98.858l-37.28-37.28 c-1.875-1.875-4.419-2.929-7.071-2.929c-2.652,0-5.196,1.054-7.071,2.929l-179.769,179.77c-3.905,3.905-3.905,10.237,0,14.143 c1.953,1.951,4.512,2.927,7.071,2.927s5.119-0.976,7.071-2.929L289.115,102.79l37.28,37.28c3.905,3.905,10.237,3.905,14.143,0 L446.466,34.143v33.81c0,5.523,4.477,10,10,10s10-4.477,10-10V11C466.466,10.837,466.449,10.678,466.442,10.516z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <circle cx="75.64" cy="303.31" r="10"></circle>
                                    </g>
                                </g>
                            </svg>
                            <h3 class="work-title h5 mb-0"><b>Small &amp; Medium Business</b></h3>
                        </div>
                    </div>
                    <div class="col-6 col-sm-6 col-md-6 col-xl-6 my-3">
                        <div class="card-box p-3 p-md-4 text-center">
                            <svg class="mb-3" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 480 480" style="enable-background:new 0 0 480 480" xml:space="preserve" height="60px" width="60px" fill="#cb1ac5">
                                <g>
                                    <g>
                                        <rect x="168" y="160" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="168" y="192" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="168" y="224" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="168" y="256" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="200" y="160" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="200" y="192" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="200" y="224" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="200" y="256" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="232" y="160" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="232" y="192" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="232" y="224" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="232" y="256" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="264" y="160" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="200" y="128" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="232" y="128" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="264" y="128" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="264" y="192" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="264" y="224" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="264" y="256" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="296" y="160" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="296" y="192" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="296" y="224" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="296" y="256" width="16" height="16"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M472.009,120c-0.003,0-0.006,0-0.009,0H352V8c0.003-4.416-3.575-7.997-7.991-8c-0.003,0-0.006,0-0.009,0H136 c-4.416-0.003-7.997,3.575-8,7.991c0,0.003,0,0.006,0,0.009v112H8c-4.416-0.003-7.997,3.575-8,7.991c0,0.003,0,0.006,0,0.009v288 c-0.003,4.416,3.575,7.997,7.991,8c0.003,0,0.006,0,0.009,0h16.723c-0.464,2.642-0.706,5.318-0.723,8v40 c-0.003,4.416,3.575,7.997,7.991,8c0.003,0,0.006,0,0.009,0h416c4.416,0.003,7.997-3.575,8-7.991c0-0.003,0-0.006,0-0.009v-40 c-0.016-2.682-0.258-5.358-0.723-8H472c4.416,0.003,7.997-3.575,8-7.991c0-0.003,0-0.006,0-0.009V128 C480.003,123.584,476.425,120.003,472.009,120z M344,344c0,8.837-7.163,16-16,16s-16-7.163-16-16s7.163-16,16-16 C336.832,328.011,343.989,335.168,344,344z M152,328c8.837,0,16,7.163,16,16s-7.163,16-16,16s-16-7.163-16-16 C136.011,335.168,143.168,328.011,152,328z M104,464H40v-32c0-17.673,14.327-32,32-32s32,14.327,32,32V464z M56,368 c0-8.837,7.163-16,16-16s16,7.163,16,16s-7.163,16-16,16C63.168,383.989,56.011,376.832,56,368z M105.212,397.425 c-3.017-2.91-6.4-5.414-10.064-7.449c12.202-12.784,11.731-33.04-1.053-45.243s-33.04-11.731-45.243,1.053 c-11.803,12.365-11.803,31.824,0,44.189c-7.636,4.238-13.98,10.467-18.358,18.024H16V136h112v187.056 c-10.986,12.327-10.615,31.039,0.852,42.92C116.852,372.613,108.253,384.053,105.212,397.425z M184,464h-64v-56 c0-17.673,14.327-32,32-32s32,14.327,32,32V464z M280,464h-80v-56c0-22.091,17.909-40,40-40c22.091,0,40,17.909,40,40V464z M216,328c0-13.255,10.745-24,24-24s24,10.745,24,24s-10.745,24-24,24C226.751,351.986,216.014,341.249,216,328z M288.722,380.495 c-5.357-9.444-13.327-17.139-22.953-22.16c16.897-14.232,19.057-39.467,4.826-56.364c-14.232-16.897-39.467-19.057-56.364-4.826 c-16.897,14.232-19.057,39.467-4.826,56.364c1.468,1.743,3.082,3.357,4.826,4.826c-9.626,5.021-17.596,12.716-22.953,22.16 c-4.206-6-9.722-10.965-16.13-14.519c12.265-12.547,12.037-32.662-0.511-44.927c-8.079-7.898-19.744-10.91-30.637-7.913V16h192 v297.136c-2.607-0.719-5.296-1.101-8-1.136c-17.626-0.047-31.953,14.204-32,31.831c-0.022,8.248,3.151,16.185,8.852,22.145 C298.444,369.53,292.928,374.495,288.722,380.495z M360,432v32h-64v-56c0-17.673,14.327-32,32-32c17.673,0,32,14.327,32,32V432z M440,464h-64v-32c0-17.673,14.327-32,32-32c17.673,0,32,14.327,32,32V464z M392,368c0-8.837,7.163-16,16-16s16,7.163,16,16 s-7.163,16-16,16C399.168,383.989,392.011,376.832,392,368z M464,408h-14.494c-4.378-7.557-10.722-13.786-18.358-18.024 c12.202-12.784,11.731-33.04-1.053-45.243c-12.784-12.202-33.04-11.731-45.243,1.053c-11.803,12.365-11.803,31.824,0,44.189 c-3.664,2.035-7.048,4.539-10.064,7.449c-3.04-13.372-11.64-24.812-23.64-31.449c11.467-11.881,11.839-30.593,0.853-42.92V136h112 V408z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="376" y="184" width="16" height="128"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="424" y="184" width="16" height="128"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="40" y="184" width="16" height="128"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="88" y="184" width="16" height="128"></rect>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M288.009,32c-0.003,0-0.006,0-0.009,0h-96c-4.416-0.003-7.997,3.575-8,7.991c0,0.003,0,0.006,0,0.009v64 c-0.003,4.416,3.575,7.997,7.991,8c0.003,0,0.006,0,0.009,0h96c4.416,0.003,7.997-3.575,8-7.991c0-0.003,0-0.006,0-0.009V40 C296.003,35.584,292.425,32.003,288.009,32z M280,96h-80V48h80V96z">
                                        </path>
                                    </g>
                                </g>
                            </svg>
                            <h3 class="work-title h5 mb-0"><b>Enterprise</b></h3>
                        </div>
                    </div>
                    <div class="col-6 col-sm-6 col-md-6 col-xl-6 my-3">
                        <div class="card-box p-3 p-md-4 text-center">
                            <svg class="mb-3" version="1.1" id="Capa_83" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 480 480" style="enable-background:new 0 0 480 480" xml:space="preserve" height="60px" width="60px" fill="#ff9800">
                                <g>
                                    <g>
                                        <path d="M72.004,120c-0.001,0-0.003,0-0.004,0H40c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C80.001,123.583,76.421,120.001,72.004,120z M64,152H48v-16h16V152z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M168.004,120c-0.001,0-0.003,0-0.004,0h-32c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C176.001,123.583,172.421,120.001,168.004,120z M160,152h-16v-16h16V152z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M264.004,120c-0.001,0-0.003,0-0.004,0h-32c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C272.001,123.583,268.421,120.001,264.004,120z M256,152h-16v-16h16V152z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M72.004,208c-0.001,0-0.003,0-0.004,0H40c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C80.001,211.583,76.421,208.001,72.004,208z M64,240H48v-16h16V240z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M168.004,208c-0.001,0-0.003,0-0.004,0h-32c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C176.001,211.583,172.421,208.001,168.004,208z M160,240h-16v-16h16V240z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M264.004,208c-0.001,0-0.003,0-0.004,0h-32c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C272.001,211.583,268.421,208.001,264.004,208z M256,240h-16v-16h16V240z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M72.004,296c-0.001,0-0.003,0-0.004,0H40c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C80.001,299.583,76.421,296.001,72.004,296z M64,328H48v-16h16V328z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M168.004,296c-0.001,0-0.003,0-0.004,0h-32c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C176.001,299.583,172.421,296.001,168.004,296z M160,328h-16v-16h16V328z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M264.004,296c-0.001,0-0.003,0-0.004,0h-32c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C272.001,299.583,268.421,296.001,264.004,296z M256,328h-16v-16h16V328z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M72.004,384c-0.001,0-0.003,0-0.004,0H40c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C80.001,387.583,76.421,384.001,72.004,384z M64,416H48v-16h16V416z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M264.004,384c-0.001,0-0.003,0-0.004,0h-32c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v32 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h32c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-32 C272.001,387.583,268.421,384.001,264.004,384z M256,416h-16v-16h16V416z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M472.004,200c-0.001,0-0.003,0-0.004,0h-80v-40.047c7.235,4.076,15.31,6.436,23.602,6.898 c0.84,0.047,1.68,0.062,2.512,0.062c13.377-0.028,26.183-5.427,35.542-14.984c3.124-3.122,3.125-8.186,0.003-11.309 c-0.001-0.001-0.002-0.002-0.003-0.003l-27.48-27.48l19.48-19.481l-11.312-11.312l-19.481,19.48l-27.48-27.48 c-3.174-3-8.138-3-11.312,0c-19.92,20.148-19.952,52.564-0.07,72.75V200h-72V96c0.001-4.417-3.579-7.999-7.996-8 c-0.001,0-0.003,0-0.004,0h-8V64c0.001-4.417-3.579-7.999-7.996-8c-0.001,0-0.003,0-0.004,0h-24V8 c0.001-4.417-3.579-7.999-7.996-8c-0.001,0-0.003,0-0.004,0H56c-4.417-0.001-7.999,3.579-8,7.996C48,7.997,48,7.999,48,8v48H24 c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v24H8c-4.417-0.001-7.999,3.579-8,7.996C0,95.997,0,95.999,0,96v376 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h464c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004V208 C480.001,203.583,476.421,200.001,472.004,200z M64,16h176v40H64V16z M32,72h240v16H32V72z M144,464h-32v-57.539 c0.007-7.982,6.475-14.452,14.457-14.461H144V464z M192,464h-32v-72h17.543c7.982,0.009,14.45,6.479,14.457,14.461V464z M288,464 h-80v-57.539c-0.018-16.814-13.643-30.441-30.457-30.461h-51.086c-16.814,0.02-30.439,13.647-30.457,30.461V464H16V104h272V464z M382.336,91.922l53.754,53.758c-5.847,3.777-12.743,5.596-19.691,5.195c-10.16-0.716-19.691-5.182-26.742-12.531 C376.352,126.653,373.275,107.139,382.336,91.922z M408,464h-48v-43.305c0.009-7.009,5.69-12.689,12.699-12.695h22.602 c7.009,0.006,12.691,5.686,12.699,12.695V464z M464,464h-40v-43.305c-0.02-15.841-12.858-28.677-28.699-28.695h-22.602 c-15.841,0.018-28.679,12.854-28.699,28.695V464h-40V216h160V464z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M408.004,232c-0.001,0-0.003,0-0.004,0h-48c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v48 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h48c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-48 C416.001,235.583,412.421,232.001,408.004,232z M400,280h-32v-32h32V280z">
                                        </path>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M408.004,312c-0.001,0-0.003,0-0.004,0h-48c-4.417-0.001-7.999,3.579-8,7.996c0,0.001,0,0.003,0,0.004v48 c-0.001,4.417,3.579,7.999,7.996,8c0.001,0,0.003,0,0.004,0h48c4.417,0.001,7.999-3.579,8-7.996c0-0.001,0-0.003,0-0.004v-48 C416.001,315.583,412.421,312.001,408.004,312z M400,360h-32v-32h32V360z">
                                        </path>
                                    </g>
                                </g>
                            </svg>
                            <h3 class="work-title h5 mb-0"><b>Agencies</b></h3>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="mob-serv pc-none">
    <div class="vid-mob-top">
        <div class="row">
            <div class="col-6 mob-btm">
                <div class="mob-box">
                    <img src="img/rocket.png" alt="">
                </div>
            </div>
            <div class="col-6 mob-btm">
                <div class="mob-box">
                    <img src="img/rocket.png" alt="">
                    <h4 class="text-center">
                        Web Development
                    </h4>
                </div>

            </div>
            <div class="col-6 mob-btm">
                <div class="mob-box">
                    <img src="img/rocket.png" alt="">
                </div>
            </div>
            <div class="col-6">

            </div>
        </div>
    </div>
</section>
<?php require('footer.php'); ?>